...
static void freeMem(Widget wg, XtPointer client, XtPointer call)
{
  XtFree(client);
}
...
/* popupMenuHandler()                      */
/* --------------------------------------- */
static void popupMenuHandler(wg, clientData, event, cont)
     Widget wg;
     XtPointer clientData;
     XEvent *event;
     Boolean *cont;
...
void addPopupMenuHandler(menu, parent)
    Widget menu;
    Widget parent;
{
