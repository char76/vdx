...
DeleteChild(Widget child)
{
    ...
    /* Sometimes the items is already NULL. In that case we return
       immediately, but the reason for this failure is somewhere else.
       So this is only a work around */

    if(tp->items == NULL)
      return;

    index = ItemToIndex(child);
#else
...
#ifndef VDX_CHANGES
    /* I think decrease is --, therefore I don't want the next line. */
    /* After UpdateItems() the item_count should have the right value ?-) */
	tp->item_count++;
#endif
