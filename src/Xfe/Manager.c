...
#ifdef VDX_CHANGES
        /* Don't reject the child, but mark the child as
           private component. The message should inform the user,
...
DeleteChild(Widget child)
{
    ...
    if (layout
#ifdef VDX_CHANGES
        /* Don't call the layout function, if self is being_destroyed */
        && _XfeIsAlive(w)
#endif
        )
    {
